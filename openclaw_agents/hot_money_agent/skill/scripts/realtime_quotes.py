...
def _to_ts_code(stock_code: str) -> str:
    """将纯数字股票代码转为tushare格式（600519 -> 600519.SH）"""
    if "." in stock_code:
        return stock_code
...
def get_realtime_quotes(stock_code: str) -> dict:
    ...
    Returns:
        dict: 实时行情数据
    """
    # 主路径：efinance（get_quote_snapshot 获取单只股票实时快照）
    if ef is not None:
        try:
        ...
        try:
            ...
            print(f"efinance获取实时行情失败({stock_code}): {e}")
    ...
    if _pro is not None:
        try:
            ...
            print(f"tushare获取实时行情失败({stock_code}): {e}")
...
if __name__ == "__main__":
    ...
    result = get_realtime_quotes(code)
